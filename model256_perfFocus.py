...
from tensorflow.keras.layers import Conv2D, Input, Concatenate, Add, UpSampling2D, Multiply, Lambda
...
def build_superresolution_model(output_size=(256, 256)):
    ...
    weight_low_res = 0.7
    weight_high_res = 0.3
    x = Lambda(lambda t: t * weight_low_res)(x)
    y = Lambda(lambda t: t * weight_high_res)(y)
...
def visualize_intermediate_outputs(model, inputs):
    ...
    for i, activation in enumerate(activations):
        if len(activation.shape) == 4:  # Only visualize 4D outputs (batch, height, width, channels)
            ...
            plt.savefig(f'intermediate_outputs/layer_{i + 1}.png')
            plt.close()